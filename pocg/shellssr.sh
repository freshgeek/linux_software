...
#	clear && echo "===================================================" && echo
	...
	echo
	echo -e " 上传\t    : ${Green_font_prefix}${u}${Font_color_suffix}"
	echo -e " 下载\t    : ${Green_font_prefix}${d}${Font_color_suffix}"
	echo -e " 总计\t    : ${Green_font_prefix}${transfer_enable_Used_2}${Font_color_suffix}"
...
match_add=$(python mujson_mgr.py -a -u "${ssr_user}" -p "${ssr_port}" -k "${ssr_password}" -m "aes-128-ctr" -O "auth_aes128_md5" -G "2" -o "plain" -s "0" -S "0" -t "838868" -f ""|grep -w "add user info")
if [[ -z "${match_add}" ]]; then
				echo -e "${Error} 用户添加失败 ${Green_font_prefix}[用户名: ${ssr_user} , 端口: ${ssr_port}]${Font_color_suffix} "
			else
				Add_iptables
...
#	echo "query port";
	View_user_port=$2
	query_port
#elif [ $action == "query_ssrlink" ]; then
#	View_user_port=$2
#	query_port
elif [ $action == "query_all" ]; then
	#statements
#	echo "query_all"
	SSR_installation_status
	List_port_user	
elif [ $action == "ip_addr" ]; then
	View_user_connection_info_1 "IP_address"
else
	echo -e "参数错误:\nadd port pw\ndelete port\nalter  port pw\nquery  port\nquery_all\nip_addr"
fi
...
set_centos_version(){
	...
	if [[ -z `cat /etc/centos-release | grep " 6"` || -z `cat /etc/redhat-release | grep " 6"` ]]; then
